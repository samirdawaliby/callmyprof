...
> **Version** : 0.2.0 | **Dernière mise à jour** : 18 Mars 2026
...
| `GET` | `/session/:bookingId` | `renderSessionPage()` | Page video embedded (Daily.co/Jitsi) |
| `GET` | `/session-demo/daily` | demo | Demo session Daily.co |
| `GET` | `/session-demo/jitsi` | demo | Demo session Jitsi |
| `GET` | `/student/:leadId?key=xxx` | `renderStudentPortal()` | Portail etudiant (magic link) |
| `GET` | `/api/webhooks/whatsapp` | `verifyWhatsAppWebhook()` | Verification webhook |
...
| `POST` | `/api/test-email` | `testEmail()` | Envoyer email de test |
...
### Done (MVP + Session 18 Mars 2026)
- [x] Landing page trilingue (EN/FR/AR) + detection geo
- [x] Formulaire CTA (8 domaines, 61 sous-domaines, 223 thematiques)
- [x] Support RTL arabe
- [x] CRM Admin complet (leads, details, suivi)
- [x] Systeme de booking (tokens, page publique, emails)
- [x] Video automatique (Daily.co + Jitsi fallback)
- [x] Onboarding tuteur 4 etapes
- [x] Gestion formateurs, familles, eleves, cours
- [x] Chatbot IA (Workers AI)
- [x] Catalogue matieres complet
- [x] Pages legales (CGU avec Capleb SAL, Privacy)
- [x] Domaine callmyprof.com en ligne
- [x] Email Resend API (envoi FR/EN/AR : nepasrepondre@ / noreply@)
- [x] Cloudflare Email Routing (reception contact@callmyprof.com)
- [x] Gmail "Send as" contact@callmyprof.com via SMTP Resend
- [x] Email confirmation lead auto + notification admin
- [x] Email design avec stepper 4 etapes
- [x] Page /session/:id avec iframe embedded (Daily.co + Jitsi branded)
- [x] Dashboard Sessions admin (/sessions) avec countdown + bouton Rejoindre
- [x] Badges UTM source sur leads (TikTok/Instagram/Facebook/Organic)
- [x] Reminder email auto 1h avant session (cron */15 min)
- [x] Portail etudiant /student/:id?key=xxx (magic link, pas de mot de passe)
- [x] Lien Discord communaute (discord.gg/Bpf6ACbWAG)

### Phase 1 - Paiements (priorite haute)
- [ ] Integration Stripe Checkout
- [ ] Webhooks Stripe pour confirmation
- [ ] Integration PayPal (alternatif)
- [ ] Page paiement admin

### Phase 2 - Factures
- [ ] Generation factures PDF
- [ ] Envoi automatique par email
- [ ] Numerotation sequentielle

### Phase 3 - Sessions ameliorees
- [ ] Rappel WhatsApp 1h avant (WhatsApp Business API)
- [ ] Enregistrement des sessions (Daily.co cloud recording)
- [ ] Feedback post-session automatique

### Phase 4 - Portail etudiant v2
- [x] Espace etudiant `/student/:id?key=xxx`
- [x] Voir ses sessions a venir / passees
